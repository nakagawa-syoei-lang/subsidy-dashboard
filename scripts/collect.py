...
def fetch_subsidies(keyword="", limit=100, offset=0):
    ...
    try:
        ...
        logger.info(f"  Status: {res.status_code} URL: {res.url[:120]}")
        if res.status_code != 200:
            logger.warning(f"  Error: {res.text[:300]}")
            return None
        return res.json()
...
def main():
    ...
    # 全件取得（acceptance=1で受付中のみ）
    logger.info("=== 全件取得 ===")
    offset = 0
    while True:
        data = fetch_subsidies(limit=100, offset=offset)
        if not data:
            break
        ...
        logger.info(f"  取得済: {len(all_items)}件 / 全{total}件")
    ...
    pref_counts = Counter(x["pref"] for x in all_items)
    logger.info(f"地域別: {dict(pref_counts.most_common(10))}")
    ...
    logger.info(f"1都3県: {len(kanto)}件")
