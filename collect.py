...
# -*- coding: utf-8 -*-
"""
補助金・助成金 収集スクリプト（GitHub Actions用）
収集結果を docs/data.json に保存 → GitHub Pages で表示
"""
...
def fetch(url, timeout=12):
    try:
        ...
        if "xml" in content_type or url.endswith((".xml", ".rdf", ".rss")):
...
def extract_detail(url):
    ...
    for pat in [
        ...
        if m:
            detail["amount"] = m.group(0).strip()[:60]
            break

    for pat in [
        r"(締切日?[^\n]{0,5}[：:]\s*(?:令和|平成)?\d+年\d{1,2}月\d{1,2}日)",
        r"(申請期限[^\n]{0,5}[：:]\s*[^\n]{5,50})",
        r"(募集期間[^\n]{0,5}[：:][^\n]{5,60})",
        ...
        m = re.search(pat, text)
        if m:
            detail["deadline"] = m.group(0).strip()[:60]
            break
    ...
    for pat in [
        ...
        m = re.search(pat, text)
        if m:
            detail["target"] = m.group(0).strip()[:80]
            break
...
def classify(title):
    mapping = {
        "IT・デジタル":     ["IT", "DX", "デジタル", "AI", "クラウド", "ICT"],
        "雇用・人材":       ["雇用", "人材", "採用", "訓練", "両立", "賃上げ", "賃金"],
        "設備・機械":       ["設備", "機械", "装置", "工場", "製造"],
        "創業・起業":       ["創業", "起業", "スタートアップ", "開業"],
        "販路拡大":         ["販路", "輸出", "海外", "EC", "展示会"],
        "省エネ・環境":     ["省エネ", "環境", "脱炭素", "再生可能", "GX"],
        "研究開発":         ["研究", "開発", "技術", "イノベーション"],
        "融資・貸付":       ["融資", "貸付", "ローン", "資金調達"],
        "事業再構築":       ["再構築", "転換", "新事業", "多角化"],
        "物価・光熱費対策": ["物価", "光熱費", "エネルギー", "電気代", "燃料"],
...
def collect_html(name, pref, base_url, pattern=None):
    ...
    for a in links:
        ...
        if not title or len(title) < 6 or not is_subsidy(title):
            continue
        href = a.get("href", "")
...
def make_row(title, org, pref, url, detail):
    return {
        "id": re.sub(r"\W", "", title[:20]) + str(abs(hash(url)) % 10000),
        "title":    title[:120],
        "org":      org,
        "pref":     pref,
        ...
        "url":      url,
        "source":   "自治体" if pref not in ("全国",) else "国・省庁",
        "date":     str(date.today()),
    }
...
RSS_SOURCES = [
    ("経済産業省",  "全国", "https://www.meti.go.jp/feed/topics.rdf"),
    ("厚生労働省",  "全国", "https://www.mhlw.go.jp/rss/topics.rdf"),
    ("中小企業庁",  "全国", "https://www.chusho.meti.go.jp/rss/news.rdf"),
    ("総務省",      "全国", "https://www.soumu.go.jp/menu_kyotsuu/whatsnew/rss.xml"),
    ("神奈川県",    "神奈川県", "https://www.pref.kanagawa.jp/prs/list.xml"),
    ("大阪府",      "大阪府",   "https://www.pref.osaka.lg.jp/rss/hodo/index.rdf"),
    ("愛知県",      "愛知県",   "https://www.pref.aichi.jp/rss/news/index.rdf"),
    ("福岡県",      "福岡県",   "https://www.pref.fukuoka.lg.jp/rss/news.rdf"),
    ("埼玉県",      "埼玉県",   "https://www.pref.saitama.lg.jp/rss/atom/newsrelease.xml"),
    ("千葉県",      "千葉県",   "https://www.pref.chiba.lg.jp/rss/news.xml"),
    ("兵庫県",      "兵庫県",   "https://web.pref.hyogo.lg.jp/rss/news.rdf"),
    ("広島県",      "広島県",   "https://www.pref.hiroshima.lg.jp/rss/news.rdf"),
    ("北海道",      "北海道",   "https://www.pref.hokkaido.lg.jp/rss/news.rdf"),
    ("静岡県",      "静岡県",   "https://www.pref.shizuoka.jp/rss/atom/top.xml"),
    ("京都府",      "京都府",   "https://www.pref.kyoto.jp/rss/index.rdf"),
    ("宮城県",      "宮城県",   "https://www.pref.miyagi.jp/rss/news.rdf"),
]

HTML_SOURCES = [
    ("東京都産業労働局", "東京都",
     "https://www.sangyo-rodo.metro.tokyo.lg.jp/news/", r"/news/"),
    ("東京中小企業振興公社", "東京都",
     "https://www.tokyo-kosha.or.jp/support/josei/index.html", r"/support/josei/"),
    ("ミラサポplus",  "全国",
     "https://mirasapo-plus.go.jp/subsidy/", r"/subsidy/\d+"),
    ("J-Net21",       "全国",
     "https://j-net21.smrj.go.jp/snavi/articles?category=C0302", r"/snavi/articles/\d+"),
    ("補助金ポータル", "全国",
     "https://hojyokin-portal.jp/columns/sme_subsidy", None),
]
...
def main():
    ...
    logger.info(f"収集完了: {len(unique)}件（重複排除後）")
